...
namespace ProcessorEmulator.Core
{
    ...
    public static class CeRomTocFiles
    {
        ...
        // Do not leftover hop. Do not invent dest.
        // Live b66258b leftover-wait99-wrap-cont pc=
        // 0x03F71720 word=0x8EC20000 s6=0x01FFFCA4
        // plant=0x03F74844 then leftover-wait99-wrap-
        // halt pc=0x03F71734 word=0x2402F9A2. wrap-
        // cont ran dump lw $v0,0($s6). leftover dest
        // leftover-syscall addiu $v0,$0,-1630 wrap-
        // halt correct. Dump dest leftover-syscall
        // -1630 wrapper 0x80095720: 0x80095710 lui
        // $v0,0x200; 0x8009571C addiu $s6,$v0,-860
        // → $s6=0x01FFFCA4 (GetProc cache / same
        // ProcessInfoFaultVa). 0x80095720 lw $v0,
        // 0($s6) 0x8EC20000; 0x80095724 beq $v0,$0,
        // +3 0x10400003; 0x8009572C b +2 0x10000002;
        // 0x80095730 lw $v0,608($v0) 0x8C420260
        // (real GetProc); 0x80095734 addiu $v0,$0,
        // -1630 leftover-syscall fallback;
        // 0x80095738 jalr $v0 0x0040F809. leftover
        // dest useg 0x03F71720 is dest wrapper
        // overlay (same page offset). leftover dest
        // dest-wrapper success does not enter
        // leftover-syscall -1630. leftover dest
        ...
        public const uint LeftoverWait99WrapBeq = 0x03F71724;
        public const uint LeftoverWait99WrapBeqWord = 0x10400003;
        public const uint LeftoverWait99WrapSkip = 0x03F7172C;
        public const uint LeftoverWait99WrapSkipWord = 0x10000002;
        public const uint LeftoverWait99WrapGetProcLw = 0x03F71730;
        public const uint LeftoverWait99WrapGetProcLwWord = 0x8C420260;
        public const uint LeftoverWait99WrapSyscall = 0x03F71734;
        ...
        // Live 58f17ea leftover-api-54-cont dest=
        // 0x8005A6D0 leftover-wait99-wrap-halt
        // pc=0x03F71740 word=0x8FC60000 plant=
        // 0x03F74844 (refuse leftover dest leftover
        // $fp). wrap-plant dest-live GetProc
        // gp=0x8003EABC / methods[152] already
        // present. leftover $fp gate blocked
        // leftover-wait99-wrap-cont. leftover-
        // wait99-wrap-cont at GetProc-wrapper $ra
        // when dest-live methods[152] is present
        // even if leftover $fp is leftover dest.
        // leftover dest leftover-syscall -1630 /
        // missing GetProc wrap-halt stays. leftover
        ...
        // Live 41b58dc leftover-wait99-wrap-need slot=
        // kdata=ahsys=methods=getproc=0 via=empty then
        // wrap-halt -1630; LoadO32-ret v0=0. ahSys[0]
        // unset / not the live Win32 CINFO at wrap.
        ...
        // Live 05a9778 leftover-ret +4 leftover
        // dest before this lw. Dump leftover
        // ObjectCall 0x80039148 (sole jal
        // 0x80015980): 0x800391C4 lw $t3,16($fp)
        // is leftover 0x8001597C $ra; 0x800391CC
        // sw $t3,4($s7) writes frame+4; 0x80039218
        // sw $s7,24(thread) links it. dest of
        // leftover dest 0x03F71740 is mid-hash
        // (0x80089740 addu), not jalr+8. Name
        ...
        // Live 84e6a7f leftover-ret +4 leftover dest
        // 0x03F70830 (not 0x03F71740). Dump dest
        // 0x80088830 is jr $ra delay addiu $sp,40
        // of leftover-syscall -938: 0x80088810
        // addiu $t2,$0,-938; 0x80088820 jalr $t2.
        // jalr+8 is 0x80088828. EPC 0xFFFFFC56;
        // (EPC+0x3FE)>>2 = 0x15. leftover dest of
        // jalr+8 is 0x03F70828. Same leftover dest
        // $ra class. Store dest jalr+8. Do not
        // leftover hop.
        public const uint LeftoverApi938 = 0x00000015;
        public const uint LeftoverApi938Ret = 0x80088828;
        public const uint LeftoverApi938RaLo = 0x03F70820;
        public const uint LeftoverApi938RaHi = 0x03F70834;
        // Live f99af0d GetProc plant win then leftover-cstk
        // api=0xFFFFFFB2 (-78) +4 leftover dest 0x03F70830.
        // IsLeftoverApi938Ra planted LeftoverApi938Ret
        // 0x80088828 (dump leftover-syscall -938 jalr+8),
        // then leftover-halt dest stub dest=0. api -78 is
        // not -938: dump index api=(EPC+0x3FE)>>2 so
        // imm=(api<<2)-0x3FE = -1334; methods[78] is
        // ppfnMethods+0x138. Do not leftover hop to
        // 0x80088828 for api -78. leftover dest GetProc
        // dest leftover hop forbidden. Do not leftover
        // hop. Do not invent dest.
        // Live 509dd8f leftover-api-78-need refuse=
        // 0x80094830 m78=0 then leftover-api-78-halt
        // jalr8=0x80094830 m78=0 +EC=0x8003FD04.
        // leftover dest was stays destPlant after
        // refuse leftover-syscall jalr+8. leftover-
        // api-78-cont required !IsJalCalleeMid(+EC,
        // +DC). Live f99af0d +DC=0x800315F0 is
        // leftover-dispatch $ra; +DC-8 jal target is
        // not +EC, so JalCalleeMid hid dump-true
        // thread+0xEC. leftover-api-78-cont replays
        // that +EC when sane. wrap-plant via=ptr
        // 0x8005D400 filled the table pointer /
        // GetProc slot, not methods[78]. Fill +0x138
        // from dump nk.bin / a second dump-pair
        // table with dest-live GetProc. Do not plant
        // +EC as methods[78]. leftover dest GetProc
        // dest leftover hop forbidden. Do not leftover
        // hop. Do not invent dest.
        public const uint LeftoverApi78 = 0xFFFFFFB2;
        public const int LeftoverApi78Imm = -1334;
            ...
            ref uint pc)
        {
            ...
            bool wrapBeq = pc == LeftoverWait99WrapBeq
                && word == LeftoverWait99WrapBeqWord;
            bool wrapSkip = pc == LeftoverWait99WrapSkip
                && word == LeftoverWait99WrapSkipWord;
            bool wrapGetProcLw = pc == LeftoverWait99WrapGetProcLw
                && word == LeftoverWait99WrapGetProcLwWord;
        ...
        // Dump dest leftover-syscall wrapper success
        // after leftover dest GetProc lw $v0,0($s6):
        // $s6=0x01FFFCA4 (lui 0x200 + addiu -860).
        // $v0=*0x01FFFCA4 is GetProc cache. dest-live
            ...
            uint[] regs, ref uint pc)
        {
            ...
            uint lwWord = 0;
            uint jalrWord = 0;
            if (!TryPeekWord(bus, LeftoverWait99WrapSkip, out skipWord)
                || !TryPeekWord(bus, LeftoverWait99WrapGetProcLw, out lwWord)
                || !TryPeekWord(bus, LeftoverWait99WrapJalr, out jalrWord))
                return false;
            if (lwWord != LeftoverWait99WrapGetProcLwWord
                || jalrWord != LeftoverWait99WrapJalrWord)
                return false;
            uint getproc = 0;
            ...
            else
            {
                if (regs == null || regs.Length <= 2)
                    return false;
                regs[2] = getproc;
                dest = LeftoverWait99WrapJalr;
                destWord = jalrWord;
            }
            if (dest == pc)
                return false;
            pc = dest;
            TryNoteLeftoverWait99WrapCont(bus, regs, dest, destWord, cache,
                getproc);
            return true;
        }

        private static bool IsDumpWait99GetProcDest(uint va)
        {
        ...
        private static bool TryScanDumpWn32Range(MipsBus bus, uint lo, uint hi,
            out uint methods, out uint getproc, out uint wn32, out uint hop)
        {
            methods = 0;
            getproc = 0;
        ...
        // Live 509dd8f leftover-api-78-halt +EC=
        ...
        // 0x8005D400 gp=0x8003EABC. methods[78] at
        // +0x138 stayed 0: TryEnsureWait99DumpTableLive
        // returns once GetProc is dest-live and never
        // copies slot 78. Dump nk.bin B000FF at the
        // same table, or a second dump-pair table
        // with dest-live GetProc, holds the Win32
        // entry. Do not plant leftover dest / leftover
        // hop GetProc dest / +EC. leftover dest
        // GetProc dest leftover hop forbidden. Do
        // not leftover hop. Do not invent dest.
        ...
        private static bool TryResolveLeftoverApi78Table(MipsBus bus,
            out uint table)
        {
            table = 0;
            if (bus == null)
            ...
            TryPeekWord(bus, ProcessInfoFaultVa, out slot);
            TryPeekWord(bus, LeftoverWait99CacheKdata, out kdata);
            if (TryAcceptLeftoverWait99GetProcTable(bus, slot, out gp))
                table = slot;
            else if (TryAcceptLeftoverWait99GetProcTable(bus, kdata, out gp))
                table = kdata;
            else
                return false;
            ...
            uint slotOff, out uint fn)
        {
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                List<Wait99DumpRec> recs = new List<Wait99DumpRec>();
                if (data != null && TryLoadWait99DumpRecs(data, recs))
                {
                    uint destLo = LeftoverDestKseg;
                    uint destHi = LeftoverDestKseg
                        + (LeftoverDestHi - LeftoverDestLo);
                    for (int r = 0; r < recs.Count; r++)
                    {
                        Wait99DumpRec rec = recs[r];
                        if (rec.Data == null || rec.Data.Length < 4)
                        ...
                        for (int off = 0; off < max; off += 4)
                        {
                            ...
                            uint g = 0;
                            uint h = 0;
                            if (!TryGuessWait99TableFromDump(recs, hitVa, word,
                                out m, out g, out h))
                                continue;
            ...
            uint nkHi = NkCopy0Dst + NkCopy0DestLen;
            uint destLo2 = LeftoverDestKseg;
            uint destHi2 = LeftoverDestKseg + (LeftoverDestHi - LeftoverDestLo);
            for (uint va = LeftoverWait99NkImage; va + 4 < nkHi; va += 4)
            {
                if (va >= destLo2 && va < destHi2)
                {
                    va = destHi2 - 4;
                    continue;
                }
        ...
        // Live f99af0d leftover-wait99-wrap-plant
        // via=ptr now=0x8005D400. methods[78] is
        // *(ppfnMethods+0x138). Peek only; do not
        // invent dest from an empty slot.
        private static bool TryPeekLeftoverWait99Method(MipsBus bus, uint index,
            out uint fn)
        {
            fn = 0;
            if (bus == null || index > 255)
                return false;
            uint table = 0;
            uint slot = 0;
            uint kdata = 0;
            ...
            if (TryAcceptLeftoverWait99GetProcTable(bus, slot, out gp))
                ...
                table = kdata;
            else if (slot != 0 && slot != 0xFFFFFFFFu)
                table = slot;
            else
                return false;
            if (!TryPeekWord(bus, table + index * 4, out fn))
            {
                fn = 0;
                return false;
            }
        ...
        private static void TryNoteLeftoverApi78Need(MipsBus bus, uint leftoverRa,
            uint refuse)
        {
            if (_leftoverApi78NeedLogged)
                return;
            _leftoverApi78NeedLogged = true;
            TryPlantLeftoverApi78Method(bus);
            uint m78 = 0;
            TryPeekLeftoverWait99Method(bus, LeftoverApi78Meth, out m78);
            BootLog.Write("[Hive] ExtraROM ddi_nop leftover-api-78-need api=0x" +
                LeftoverApi78.ToString("X8") +
                " imm=" + LeftoverApi78Imm +
            ...
            uint[] regs, uint pc, uint word)
        {
            ...
            TryNoteLeftoverWait99WrapCont(bus, regs, pc, word, cache, getproc);
        }

        private static void TryNoteLeftoverWait99WrapCont(MipsBus bus,
            uint[] regs, uint pc, uint word, uint cache, uint getproc)
        {
            bool getprocCont = pc == LeftoverWait99WrapSkip
                || pc == LeftoverWait99WrapJalr
                || pc == LeftoverWait99WrapGetProcLw;
            if (getprocCont)
            {
                if (_leftoverWait99WrapGetProcLogged)
                    return;
                _leftoverWait99WrapGetProcLogged = true;
            }
            else if (_leftoverWait99WrapContLogged)
                return;
            else
            ...
            BootLog.Write("[Hive] ExtraROM ddi_nop leftover-wait99-wrap-cont pc=0x" +
                ...
                " plant=0x" + plant.ToString("X8") +
                (getprocCont
                    ? " (dump dest leftover-syscall wrapper success b +2 / lw $v0,608($v0) real GetProc / jalr; leftover dest leftover-syscall -1630 wrap-halt stays; leftover dest GetProc dest leftover hop forbidden; do not leftover dest)"
                    : " (dump leftover dest GetProc leftover dest lw $v0,0($s6) $s6=0x01FFFCA4 GetProc cache; leftover dest dest-wrapper success wrap-cont; leftover dest leftover-syscall wrap-halt stays; leftover dest GetProc dest leftover hop forbidden; do not leftover dest)"));
        }
            ...
            uint ra, uint destWord, uint raWord)
        {
            ...
            BootLog.Write("[Hive] ExtraROM ddi_nop leftover-wait99-why plant=0x" +
                ...
                " w2=0x" + w2.ToString("X8") +
                " w3=0x" + w3.ToString("X8") +
                " (dump leftover dest GetProc leftover dest lw $v0,0($s6) at wrap $s6=0x01FFFCA4 GetProc cache; dump dest leftover-syscall wrapper success lw $v0,608($v0) / jalr; leftover dest leftover-syscall wrap-halt stays; leftover dest GetProc dest leftover hop forbidden; do not leftover dest)");
        }

        // Live 05a9778 leftover-ret frame+4
        // leftover dest already. Dump who wrote
        // it: 0x800391CC sw leftover $ra during
        // leftover-syscall ObjectCall. api is
        // 0($fp) leftover-syscall index. One
        // Hive line. leftover-ret / leftover-
        // skip / leftover-halt stay. Do not
        // leftover hop. Do not invent dest.
        public static void TryNoteLeftoverCstkObserve(MipsBus bus, uint[] regs,
            uint pc)
        {
            if (_leftoverCstkLogged)
                return;
            ...
            if (!IsLeftoverDestVa(t3))
                return;
            _leftoverCstkLogged = true;
            uint frame = PeekGpr(regs, 23);
            uint fp = PeekGpr(regs, 30);
            uint api = 0;
            uint plus18 = 0;
            uint thr = 0;
            ...
            _leftoverCstkApi = api;
            if (TryPeekWord(bus, ThreadPtr, out thr) && thr != 0
                && thr != 0xFFFFFFFFu)
                TryPeekWord(bus, thr + ThreadSyscallFrame, out plus18);
            BootLog.Write("[Hive] ExtraROM ddi_nop leftover-cstk sw=0x" +
                LeftoverCstkSw.ToString("X8") +
        ...
        // leftover dest store with a dump jalr+8.
        // Live 84e6a7f leftover dest +4 0x03F70830
        // is leftover-syscall -938 dest wrapper
        // (jalr+8 0x80088828), not mid-hash.
        // Live f99af0d leftover-cstk api=0xFFFFFFB2
        // (-78) +4 leftover dest 0x03F70830. The
        // 938 RA range planted LeftoverApi938Ret
        // 0x80088828 then leftover-halt dest stub.
        // api -78 is leftover-syscall -1334 /
        // methods[78], not -938. Do not leftover
        // hop to 0x80088828. leftover dest GetProc
        // dest leftover hop forbidden. leftover-
        // api-78-need refuses leftover-syscall
        // jalr+8; leftover-api-78-cont replays
            ...
            uint leftoverRa, out uint dest)
        {
            ...
            // addiu $0,imm; jalr → jalr+8. Live
            // f99af0d leftover dest 0x03F70830 is
            // also leftover-syscall -938 jalr delay;
            // api -78 must not plant 0x80088828.
            // Named api / dest-ROM scan first. 938
            // RA range only when api is 0 or 938.
            // Do not leftover hop. Do not invent dest.
            if (api == LeftoverApi1630)
                ...
                out dest))
                return true;
            else if ((api == 0 || api == LeftoverApi938)
                && IsLeftoverApi938Ra(leftoverRa))
                dest = LeftoverApi938Ret;
            else
                return false;
        ...
        // before 0x800397F8 lw $s3. Live f99af0d
        // leftover-ret +4 leftover dest 0x03F70830
            ...
            uint pc)
        {
            if (pc != LeftoverLoadS3)
                return;
            if (regs == null || regs.Length <= 4 || bus == null)
                return;
            uint frame = PeekGpr(regs, 4);
            uint frame4 = 0;
            if (frame == 0 || frame == 0xFFFFFFFFu)
                return;
            if (!TryPeekWord(bus, frame + 4, out frame4))
                return;
            ...
            try
            {
                bus.Write32(frame + 4, dest);
            }
            catch
            {
                return;
            }
            if (_leftoverRetFixLogged)
                return;
            _leftoverRetFixLogged = true;
            BootLog.Write("[Hive] ExtraROM ddi_nop leftover-ret-fix was=0x" +
                frame4.ToString("X8") +
                " now=0x" + dest.ToString("X8") +
                " a0=0x" + frame.ToString("X8") +
                " (dump jalr+8 of leftover-syscall; do not leftover dest)");
        }
